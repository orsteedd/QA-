...
function ReportsPage({ dateRange = 'Last 30 days', compactTables = false, autoRefresh = true, refreshInterval = '30s' }) {
      ...
      <section className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5 shadow-[var(--depth-1)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-secondary)]">Scope: {dateRange} • Auto refresh: {autoRefresh ? `On (${refreshInterval})` : 'Off'}</p>
          </div>
      ...
      <section className="overflow-auto rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] shadow-[var(--depth-1)]">
        <table className="w-full min-w-[760px] border-collapse text-left text-sm">
          <thead className="bg-[var(--bg-elevated)] text-xs uppercase tracking-[0.08em] text-[var(--text-muted)]">
            <tr>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Report ID</th>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Name</th>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Type</th>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Generated Date</th>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Status</th>
              <th className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>Action</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-[var(--border-subtle)] text-[var(--text-secondary)]">
                <td className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>{row.id}</td>
                <td className={compactTables ? 'px-3 py-1.5 font-medium text-[var(--text-primary)]' : 'px-3 py-2 font-medium text-[var(--text-primary)]'}>{row.name}</td>
                <td className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>{row.type}</td>
                <td className={compactTables ? 'px-3 py-1.5' : 'px-3 py-2'}>{row.date}</td>
